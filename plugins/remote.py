...
async def rpromote(client, message: Message):
    try:
        ...
        group_id = args[1]
        admin_tag = ' '.join(args[2:]) if len(args) > 2 else "TSG"
    ...
    try:
        ...
        await TSG.edit("Error: I need to be an admin to promote you.")
    except UserNotParticipant:
        await TSG.edit("Error: You must be a member of the group to be promoted.")
    except RPCError as e:
        await TSG.edit(f"An error occurred: {str(e)}")
...
async def rdemote(client, message: Message):
    ...
    try:
        ...
        await TSG.edit(f"Successfully demoted {message.from_user.mention} in the group <code>{group_id}</code>.")
    
    except ChatAdminRequired:
        await TSG.edit("Error: I need to be an admin to demote you.")
    except RPCError as e:
        await TSG.edit(f"An error occurred: {str(e)}")
